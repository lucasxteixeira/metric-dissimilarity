...
def pair_batch(batch_size, X, Y, augments, encoded=False, size=None, device=None):
  ...
  augments : callable
      A function or callable object that applies augmentations to the input images.
      It should accept an image as input and return a dictionary with the key "image" containing the augmented image.
  size : tuple, optional
...
def _get_backbone(backbone, pretrained=True):
  ...
  # Check if backbone is already a model
  if backbone == "VGG32":
    network = VGG32()
    extra_layers = []
...
def train(X, Y, model_type, model_file, backbone,
  ...
  # Check if augments is provided
  # If not, use some default augmentations
  elif augments is None:
    
    # Check if patch_size is set to None
...
def compute_prototypes(embeddings, Y, n_prototypes=5, method="kmeans++", cache="prototypes.pkl"):
  ...
  method : str, optional
      The clustering method to use for computing prototypes. Options are 'kmeans', 'kmeans++', 'spectral', and 'hierarchical'. Defaults to 'kmeans'.
  cache : str, optional
  ...
  for idx, cls in enumerate(uniq_classes):
    ...
    if method == "kmeans":
      ...
      raise ValueError("Unsupported clustering method. Choose from 'kmeans', 'k-means++', 'spectral', or 'hierarchical'.")
...
def vector_to_class(probs, y_true, y_prot):
  ...
  probs = np.reshape(probs[:, 1], (y_true.shape[0], -1)) # (n_samples, n_prototypes)
  ...
  probs = np.reshape(probs, (y_true.shape[0], -1, prot_per_class)) # (n_samples, n_prototypes, n_prototypes_per_class)
  probs = np.max(probs, axis=-1) # (n_samples, n_prototypes)

  # Get the class with the highest probability for each test sample
  preds = np.argmax(probs, axis=1) # (n_samples,)
